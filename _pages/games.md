...
(function () {
  ...
  // crop to the shape's actual occupied extent — the walk's bounding box can include
  // empty rows/columns it never reached, which otherwise renders as dead space
  function walkToPuzzle(walk) {
    ...
    var path = xyPath.map(function (xy) {
  ...
  function generatePuzzle(seed) {
    ...
    if (rng() < 0.5) best.path = best.path.slice().reverse();
    return best;
  }
