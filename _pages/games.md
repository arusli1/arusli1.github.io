...
<style>
  ...
  .post-header .post-description.zp-count-pulse {
  ...
  /* while actively dragging, the line's reveal transition runs much faster —
     keeps it smooth instead of a hard instant snap, without visibly trailing
     behind a moving finger the way the full 0.18s does. The eyes stay at
     their normal (slightly slower) speed — they're decorative, not something
     that needs to track the finger 1:1, and matching the line's fast speed
     made them read as twitchy rather than a smooth "look here" */
...
(function () {
  ...
  // MAX_BOARD_W/H bound the walk's own size AND the final size after a forced
  // tail is grown on (the tail can extend the shape in any direction) — both
  // need to respect these, or a board can end up too wide to fit phone width,
  // or tall enough to force vertical scrolling (11+ rows is where that starts).
  var MAX_BOARD_W = 9;
  var MAX_BOARD_H = 10;
  ...
  function attemptWalk(rng) {
    ...
    var H = MAX_BOARD_H - 4 + Math.floor(rng() * 3);
  ...
  var lastSolvedCount = null;

  function renderSolvedCount(n) {
    if (!els.heading) return;
    var changed = lastSolvedCount !== null && n !== lastSolvedCount;
    lastSolvedCount = n;
    els.heading.textContent = headingBase + " found: " + n.toLocaleString();
    // brief color flash on an actual change (not on first load) — makes a
    // background poll picking up someone else's solve feel noticed, not just
    // silently different next time you glance at it
    if (changed) {
      els.heading.classList.remove("zp-count-pulse");
      void els.heading.offsetWidth; // restart the transition even if it's still mid-fade
      els.heading.classList.add("zp-count-pulse");
      setTimeout(function () {
        els.heading.classList.remove("zp-count-pulse");
      }, 50);
    }
